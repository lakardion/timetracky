      ...
      <form className="relative flex flex-col justify-center gap-2 rounded-lg bg-gray-700 p-4 pb-5 text-white" onSubmit={handleSubmit(onSubmit)}>
        ...
        <section aria-label="hour date" className="flex flex-wrap gap-3">
          <div className="flex flex-grow flex-col gap-2">
            <label className="font-medium" htmlFor="value">
              Value
            </label>
        ...
        <label className="font-medium" htmlFor="projectId">
          Project
        </label>
        <Controller
          control={control}
        ...
        <TextArea {...register('description')} className="flex-grow" placeholder="Describe the activity..." />
        ...
        <section className="flex justify-center gap-3 p-4">
          <Button className="flex-grow" isLoading={isHourEditing || isHourCreating} type="submit">
            {hourId ? 'Edit' : 'Add'}
          </Button>
...
const HourCalendarContainer: FC<{
  ...
  return (
    <section aria-label="calendar" className=" flex justify-center lg:basis-8/12">
      <HoursCalendar events={events} selected={datesSelected} onRangeChange={handleRangeChange} onSelectedChange={handleDateSelected} />
    </section>
  );
};
...
const HoursMain = () => {
    ...
    const noProjectsMessage =
      user && user.roleType === RoleType.ADMIN ? (
        <>
          go to{' '}
          <Link href="/projects">
            <button className="text-blue-600 visited:text-purple-600 hover:underline" type="button">
              Projects
            </button>
          </Link>{' '}
          and add one
  ...
  return (
    <>
      ...
      <section aria-label="hours and calendar container" className="flex flex-col gap-2 md:flex-grow lg:flex-row-reverse 2xl:flex-row-reverse">
        ...
        <section aria-label="hours list" className="relative flex flex-col items-center justify-start md:flex-grow md:basis-4/12">
          <HourList dateFilter={dateFilter} selectedHourId={editingHourId} onHourDelete={onHourDelete} onHourEdit={onHourEdit} />
        </section>
        {showConfirmationModal ? (
          <Modal onBackdropClick={handleCloseConfirm}>
            <ConfirmForm body="Are you sure you want to delete an hour?" errorMessage={deleteError?.message} isConfirming={isDeleting} onCancel={handleCloseConfirm} onConfirm={handleSubmitDelete} />
          </Modal>
        ) : null}
      </section>
    </>
